...
describe('StationPtyProvider', () => {
  ...
  it('preserves UTF-8 output split across Station websocket frames', async () => {
    const handler = vi.fn()
    provider.onData(handler)
    const { id } = await provider.spawn({ cols: 80, rows: 24 })
    ...
    socket.emit('message', emoji.subarray(2), true)

    expect(handler).toHaveBeenCalledTimes(1)
    expect(handler).toHaveBeenCalledWith({ id, data: '🙂' })
  })

  it('flushes incomplete UTF-8 output when the active Station websocket closes', async () => {
    const handler = vi.fn()
    provider.onData(handler)
    const { id } = await provider.spawn({ cols: 80, rows: 24 })
    const emoji = Buffer.from('🙂', 'utf8')
  ...
  it('keeps forwarding Station output when a thrown listener value cannot stringify', async () => {
    const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})
    const hostileError = {
      toString: () => {
    ...
    expect(consoleError).toHaveBeenCalledWith(
      '[station-pty] data listener failed',
      expect.objectContaining({ id, error: '[unprintable error]' })
    )
    consoleError.mockRestore()
  })
  ...
  it('deduplicates repeated PTY ids while serializing pane state', async () => {
    const { id } = await provider.spawn({ cols: 80, rows: 24, cwd: '/tmp/one' })

    const state = await provider.serialize([id, id, 'ssh:station%3Aws_123@@pty_123'])
  ...
  it('ignores remote exit status that resolves after provider disposal', async () => {
    const handler = vi.fn()
    provider.onExit(handler)
    const { id } = await provider.spawn({ cols: 80, rows: 24, cwd: '/tmp/one' })
    const status = deferredPromise<{ pty_id: string; status: 'exited'; exit_code: number }>()
  ...
  it('rejects malformed Station PTY create responses before opening a stream', async () => {
    vi.mocked(client.createPty).mockResolvedValueOnce({
      pty: {
        ...trackedPty(),
        pty_id: ''
  ...
  it('rejects Station PTY create responses without a pty object before opening a stream', async () => {
    vi.mocked(client.createPty).mockResolvedValueOnce({
      handle: {
        pty_id: 'pty_123',
        process_id: '456',
