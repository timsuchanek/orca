...
describe('StationClient', () => {
  ...
  it('rejects malformed stream-info responses before opening a WebSocket', async () => {
    ...
    fetchMock.mockResolvedValueOnce(
      okJsonResponse({
        url: 'http://127.0.0.1:18080/v1/pty/pty_123/stream',
        bearer_token: 'stream-secret-token'
      })
    )
    fetchMock.mockResolvedValueOnce(
  ...
  it('rejects null stream-info responses before opening a WebSocket', async () => {
    fetchMock.mockResolvedValueOnce(okJsonResponse(null))
    const client = new StationClient({
      baseUrl: 'http://127.0.0.1:18080',
      bearerToken: 'dtok_test:secret',
  ...
  it('trims stream-info bearer tokens before opening a WebSocket', async () => {
    fetchMock.mockResolvedValueOnce(
      okJsonResponse({
        url: 'ws://127.0.0.1:18080/v1/pty/pty_123/stream',
        bearer_token: '  stream-secret-token\n'
    ...
    await client.openPtyStream('ws_123', 'pty_123')
